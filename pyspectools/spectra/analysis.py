...
def fit_line_profile(
    ...
    params["center"].set(center, min=center * 0.9997, max=center * 1.0003)
    if logger:
        logger.debug("Guess center: {:,.4f}.".format(center))
    if intensity:
        params["height"].set(intensity, min=0.0)
    if width:
        params["sigma"].set(width, min=width * 0.95, max=width * 1.05)
    ...
    if fit_results.success is True:
        ...
        if len(percentage[percentage >= 5.0]) > 0:
            ...
            try:
                ...
                if logger:
                    logger.warning(
                        "Confidence intervals could not be evaluated, defaulting to standard error of mean."
                    )
        ...
        summary_dict = {
            name: ufloat(value, uncertainty)
            for value, uncertainty, name in zip(fit_values, uncer, names)
        }
...
def peak_find(
    ...
    peak_indices = peakutils.indexes(
        spec_df[int_col].to_numpy(), thres=thres, thres_abs=True, min_dist=min_dist
    )
    ...
    peak_df.update(direct_df.loc[differences >= 0.2])
    # Use 1sigma as the detection threshold; remove everything else!
    peak_df = peak_df.loc[peak_df[int_col] >= thres]
    peak_df.reset_index(drop=True, inplace=True)
    return peak_df


def search_molecule(species: str, freq_range=[0.0, 40e3], **kwargs):
    """
    ...
    """
    default_param = {"line_lists": ["CDMS", "JPL"], "export_limit": 20000}
...
def search_center_frequency(frequency: float, width=0.5):
    ...
    try:
        splat_df = Splatalogue.query_lines(
            min_freq * u.MHz, max_freq * u.MHz, line_lists=["CDMS", "JPL"]
        ).to_pandas()
        # These are the columns wanted
        columns = [
            "Species",
        ...
        if splat_df["Frequency"].isna().any():
            try:
                ...
                warnings.warn(
                    f"Problem with merging measured/predicted frequencies at {frequency:.4f}"
                )
...
def brute_harmonic_search(
    frequencies: np.ndarray, maxJ=10, dev_thres=5.0, prefilter=False
):
    """
    ...
    if prefilter is True:
        for length in [3, 4, 5]:
            ...
            combos = combos[deviation < 100.0]
            deviation = deviation[deviation < 100.0]
            _ = np.sort(deviation)[:50]  # this was named sorted_dev and unused
    ...
    for index, candidate in enumerate(candidates):
        ...
        min_rms, min_index, _, fit_values, fit_objs = fitting.harmonic_fit(
            candidate, maxJ=maxJ, verbose=False
        )
        data_list.append(
            [
                index,
    ...
    results_df = pd.DataFrame(
        data=data_list, columns=["Index", "RMS", "Frequencies", "B", "D"]
    )
...
def harmonic_finder(frequencies: np.ndarray, search=0.001, low_B=400.0, high_B=9000.0):
    """
...
def cluster_AP_analysis(
    progression_df: pd.DataFrame, sil_calc=False, refit=False, **kwargs
):
    """
    ...
    for index, label in enumerate(progression_df["Cluster indices"].unique()):
        ...
        if refit is True:
            ...
            params["B"].set(
                cluster_data[1], min=cluster_data[1] * 0.99, max=cluster_data[1] * 1.01
            )
            params["D"].set(cluster_data[2], vary=False)
            # Get values of J based on B again
            J = (unique_frequencies / cluster_data[1]) / 2
            fit = BJ_model.fit(data=unique_frequencies, J=J, params=params)
...
def find_series(combo: Tuple[float, float], frequencies: np.ndarray, search=0.005):
    ...
    J = np.arange(minJ, minJ + 20.0, 0.5)
    ...
    for guess in guess_centers:
        ...
        if len(nearest_neighbours) > 0:
            ...
            candidates.append(
                nearest_neighbours[np.argmin(np.abs(guess - nearest_neighbours))]
            )
    return candidates


def create_cluster_tests(
    cluster_dict: Dict[Any, Any], shots=25, dipole=1.0, min_dist=500.0, **kwargs
):
    """
    ...
    for cluster, subdict in cluster_dict.items():
        ...
        for cavity_freq in frequencies:
            ...
            # the DR off
            ftb_dict["drpower"] = -20
            ftb_str += fa.generate_ftb_line(cavity_freq, shots=shots, **ftb_dict)
            ...
            for dr_freq in frequencies:
                ...
                if np.abs(cavity_freq - dr_freq) > min_dist:
                    ...
                    ftb_str += fa.generate_ftb_line(
                        cavity_freq, shots=shots, **ftb_dict
                    )
                else:
                    pass
        full_str += ftb_str
...
def blank_spectrum(
    ...
    for frequency in frequencies:
        ...
        mask = new_spec[freq_col].between(frequency - window, frequency + window)
...
def correlate_experiments(*args, **kwargs):
    raise NotImplementedError("This function is currently not implemented.")


def plotly_create_experiment_comparison(
    experiments, thres_prox=0.2, index=0, filepath=None, **kwargs
):
    """
        ...
        if idx != index:
            top_traces.append(
                go.Scattergl(
                    ...
                    y=experiment.data[experiment.int_col] / experiment.session.baseline,
                    opacity=0.9,
    ...
    fig["layout"]["xaxis1"].update(
        title="Frequency (MHz)", showgrid=True, tickformat=":.0f"
    )
    for axis in ["yaxis1", "yaxis2"]:
        fig["layout"][axis].update(
            title="Signal-to-Noise", showgrid=True, tickformat=":.0f"
        )
    return fig


def bokeh_create_experiment_comparison(
    experiments, thres_prox=0.2, index=0, filepath=None, **kwargs
):
    """
    Function to create a plot comparing multiple experiments. This is a high
    ...
    """
    params = {"plot_height": 500, "plot_width": 900, "title": "Experiment Comparison"}
    ...
    params = {"plot_height": 500, "plot_width": 900, "title": "Unique Features"}
    params.update(**kwargs)
...
def line_weighting(frequency: float, catalog_frequency: float, intensity=None):
    ...
    if intensity is not None:
        weighting *= 10.0**intensity
    return weighting


def filter_spectrum(
    intensity: float, window: Union[str, np.ndarray] = "hanning", sigma=0.5
):
    """
    ...
    if isinstance(window, str):
        ...
        if window == "gaussian":
            ...
            window = lineshapes.gaussian(x, 1.0, 0.0, sigma)
        else:
