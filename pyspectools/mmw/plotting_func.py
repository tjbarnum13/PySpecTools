from plotly.offline import iplot, plot, init_notebook_mode  # noqa: F401
...
def static_comparison(frequencies, sample_on_df, sample_off_df, window_size=0.0001):
    ...
    for row_index, frequency in enumerate(frequencies):
        xrange = [frequency * (1.0 - window_size), frequency * (1.0 + window_size)]
        for col_index, df in enumerate([sample_on_df, sample_off_df]):
            slice_df = df.loc[
                (df["Frequency"] > xrange[0]) & (df["Frequency"] < xrange[1])
            ]
            ...
            axarray[row_index, col_index].plot(
                slice_df["Frequency"], slice_df["Field on"], label="Field On", alpha=0.4
            )
            axarray[row_index, col_index].plot(
                slice_df["Frequency"], slice_df["Off - On"], label="Off - On", alpha=1.0
            )
...
def id_survey(frequency, intensity, catdict):
    ...
    plots.append(
        go.Scatter(x=frequency, y=intensity, marker={"color": (0.0, 0.0, 0.0)})
    )
...
def save_plot(fig, filename):
    plot(fig, filename=filename, show_link=False, auto_open=False)
